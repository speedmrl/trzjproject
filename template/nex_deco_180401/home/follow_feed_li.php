...
<!--{loop $list['feed'] $feed}-->
	...
	<li class="cl{if $lastviewtime && $feed[dateline] > $lastviewtime} unread{/if}" id="feed_li_$feed['feedid']" onmouseover="this.className='flw_feed_hover cl'" onmouseout="this.className='cl'">
		...
		<!--{/if}-->
		<div class="nex_Gr_flw_article" {if $_GET['do'] == 'view' || $_GET[banavatar]}style="margin-left:0"{/if}>
			...
			<div class="pbm c cl nex_Gr_contants" id="original_content_$feed[feedid]" {if isset($carray[$feed['cid']])} style="display: none"{/if}>
				$content['content']
			...
			<div class="xg1 cl nex_Gr_Btms">
                ...
                <span class="z">
				<!--{if $thread[fid] != $_G[setting][followforumid] && $_G['cache']['forums'][$thread['fid']]['name']}-->
                <a href="forum.php?mod=forumdisplay&fid=$thread['fid']">$_G['cache']['forums'][$thread['fid']]['name']</a>
                <!--{/if}-->
                </span>
			</div>
			<!--{else}-->
