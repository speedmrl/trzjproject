	...
	<script type="text/javascript">
		...
		<!--{if $threadlist}-->
			...
			<!--{loop $threadlist $thread}-->
				...
				newthread[{$i}] = {'tid':$thread[tid], 'thread': {'icn':{'className':'icn','val':'<div class="nex_forumlistava"><!--{if $thread['authorid'] && $thread['author']}--><a href="home.php?mod=space&uid=$thread[authorid]" c="1"><!--{avatar($thread[authorid],small)}--></a><!--{else}-->
<img src="$_G['style']['styleimgdir']/hidden.png" title="$_G[setting][anonymoustext]" alt="$_G[setting][anonymoustext]" />
<!--{/if}-->'}<!--{if $_G['forum']['ismoderator']}-->, 'o':{'className':'o','val':'<input type="checkbox" value="{$thread[tid]}" name="moderate[]" onclick="tmodclick(this)">'}<!--{/if}--> ,'common':{'className':'new fn','val':'<div class="listt">{if !$_G['setting']['forumdisplaythreadpreview']}<a class="tdpre y" href="javascript:void(0);" onclick="javascript:previewThread(\'$thread[tid]\', \'$thread[id]\');">{lang preview}</a>{/if}$thread[threadurl]</div><div class="listb cl"><div class="listbl z"><span class="listcategory"></span><cite>$thread[authorurl]</cite>&nbsp;&nbsp;&nbsp;<em><span>{$thread[dateline]}</span></em>&nbsp;&nbsp;&nbsp;</div><div class="listbr y"><span class="llp y">{lang lastpost}&nbsp;:<cite>$thread[lastposterurl]</cite><em><a  style=" color:#a4a4a4;" href="forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost#lastpost">{$thread[lastpost]}</a></em></span></div><div class="listcount"><span class="listviews">{$thread[views]}</span><span class="listreplies">{$thread[replies]}</span></div></div></div> '},'author':{'className':'by','val':'<cite>$thread[authorurl]</cite><em><span>{$thread[dateline]}</span></em>'}, 'num':{'className':'num','val':'<a href="forum.php?mod=viewthread&tid={$thread[tid]}">{$thread[replies]}</a><em>{$thread[views]}</em>'}, 'lastpost':{'className':'by','val':'<cite>$thread[lastposterurl]</cite><em><a href="forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost#lastpost">{$thread[lastpost]}</a></em>'}}};
				<!--{eval $i++;}-->
			<!--{/loop}-->
