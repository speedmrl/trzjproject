...
<!--{else}-->
	...
	<!--{if $_GET['from'] != 'preview' && !empty($post['ratelog'])}-->
		<h3 class="psth xs1" style="display:none;"><span class="icon_ring vm"></span>{lang rate}</h3>
		<dl id="ratelog_$post[pid]" class="rate{if !empty($_G['cookie']['ratecollapse'])} rate_collapse{/if}">
			...
			<!--{/if}-->
				...
				<!--{if $_G['setting']['ratelogon']}-->
					...
					<p class="ratc xg1 cl">
						<a href="forum.php?mod=misc&action=viewratings&tid=$_G[tid]&pid=$post[pid]" onclick="showWindow('viewratings', this.href)" title="{lang rate_view}" class="xg1 y">{lang rate_view}</a>总评分&nbsp:&nbsp
							
							<!--{loop $post['ratelogextcredits'] $id $score}-->
								<!--{if $score > 0}-->
									<span>{$_G['setting']['extcredits'][$id][title]} <i><span class="xi1">+$score</span></i></span>
								<!--{else}-->
									<span>{$_G['setting']['extcredits'][$id][title]} <i><span class="xi1">$score</span></i></span>
								<!--{/if}-->
							<!--{/loop}-->
