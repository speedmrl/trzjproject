...
<div class="boardnav">
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct2{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
        ...
        <!--{else}-->
            ...
            <div class="nex_bkjsbg">
                <div class="nex_bkjsbg_top">
                    <div class="nex_bkjsbg_topimg"><img class="nex_bktu" src="$_G[forum][banner]" alt="$_G['forum'][name]" /></div>
                    <b></b>
                    <div class="nex_bkjsbg_topfav"><a href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum" id="a_favorite" class="nex_sc" title="{lang forum_favorite}" onclick="showWindow(this.id, this.href, 'get', 0);"><i class="xi1" id="number_favorite" {if !$_G[forum][favtimes]} style="display:none;"{/if}><span class="number_favorite_num">$_G[forum][favtimes]</span></i></a></div>
                </div>
                <div class="nex_bkjsbg_btm">
                    <div class="nex_bkjsbg_btmtop">
                        <div class="nex_bkicons">
                            ...
                            <!--{else}-->
                            <img src="$_G[style][styleimgdir]/forumlist/forum_icon.png" alt="$_G[forum][name]" />
                            <!--{/if}-->
                        </div>
                        <div class="nex_bkinterls">
                            <div class="nex_bkinterls_top">
                                <div class="nex_bkinterls_ls"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]">$_G['forum'][name]</a></div>
                                <div class="nex_bkinterls_ms">{lang rank}: <em title="{lang previous_rank}:$_G[forum][oldrank]">$_G[forum][rank]</em><!--{if $_G[forum][rank] <= $_G[forum][oldrank]}--><span class="nex_ico_increase"></span><!--{else}--><span class="nex_ico_fall"></span><!--{/if}--></span></div>
                                <div class="nex_bkinterls_ts">
                                <!--{if $_G['forum']['ismoderator']}-->
                                <!--{if $_G['forum']['recyclebin']}-->
                                    <a href="{if $_G['adminid'] == 1}admin.php?mod=forum&action=recyclebin&frames=yes{elseif $_G['forum']['ismoderator']}forum.php?mod=modcp&action=recyclebin&fid=$_G[fid]{/if}" class="nex_fa_bins" target="_blank">{lang forum_recyclebin}</a>
                                <!--{/if}-->
                                <!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}-->
                                <!--{if $_G['forum']['status'] != 3}-->
                                    <a class="nex_langcps" href="forum.php?mod=modcp&fid=$_G[fid]">{lang modcp}</a>
                                <!--{else}-->
                                    <a class="nex_langcps" href="forum.php?mod=group&action=manage&fid=$_G[fid]">{lang modcp}</a>
                                <!--{/if}-->
                                <!--{/if}-->	
                                <!--{/if}-->
                                </div>
                                <div class="nex_bkinterls_rs">
                                    <!--{if $moderatedby}--><div class="nex_banzhu">{lang forum_modedby}: $moderatedby</div><!--{/if}-->
                                </div>
                                <div class="clear"></div>
                            </div>
                            <div class="nex_bkinterls_btm">
                                <div class="nex_bkjs_status">
                                    <ul>
                                        <li class="nex_todayposts"><em>今日发帖</em>$_G[forum][todayposts]</li>
                                        <li class="nex_totalposts"><em>本版主题</em>$_G[forum][threads]</li>
                                        <div class="clear"></div>
                                    </ul>
                                </div>
            ...
            <div class="nex_bkjsbg_none">
                <div class="nex_nonebgpart_top">
                    ...
                    <div class="nex_nonebginterls">
                        <div class="nex_nonebginterls_top">
                            <div class="nex_nonebginterls_top_ls"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]">$_G['forum'][name]</a></div>
                            <div class="nex_nonebginterls_top_ms">{lang rank}: <em title="{lang previous_rank}:$_G[forum][oldrank]">$_G[forum][rank]</em><!--{if $_G[forum][rank] <= $_G[forum][oldrank]}--><span class="nex_ico_increase"></span><!--{else}--><span class="nex_ico_fall"></span><!--{/if}--></span></div>
                            <div class="nex_nonebginterls_top_ks">
                            <!--{if $_G['forum']['ismoderator']}-->
                            <!--{if $_G['forum']['recyclebin']}-->
                                <a href="{if $_G['adminid'] == 1}admin.php?mod=forum&action=recyclebin&frames=yes{elseif $_G['forum']['ismoderator']}forum.php?mod=modcp&action=recyclebin&fid=$_G[fid]{/if}" class="nex_fa_bin" target="_blank">{lang forum_recyclebin}</a>
                            <!--{/if}-->
                            <!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}-->
                            <!--{if $_G['forum']['status'] != 3}-->
                                <a class="nex_langcp" href="forum.php?mod=modcp&fid=$_G[fid]">{lang modcp}</a>
                            <!--{else}-->
                                <a class="nex_langcp" href="forum.php?mod=group&action=manage&fid=$_G[fid]">{lang modcp}</a>
                            <!--{/if}-->
                            <!--{/if}-->	
                            <!--{/if}-->
                            </div>
                            <div class="nex_nonebginterls_top_rs">
                                <!--{if $moderatedby}--><div class="nex_nonebanzhu">{lang forum_modedby}: $moderatedby</div><!--{/if}-->
                            </div>
                            <div class="clear"></div>
                        </div>
                        <div class="nex_nonebginterls_btm">
                            <div class="nex_nonebginterls_btml">
                                <ul>
                                    <li class="nex_todayposts"><em>今日发帖</em>$_G[forum][todayposts]</li>
                                    <li class="nex_totalposts"><em>本版主题</em>$_G[forum][threads]</li>
                                    <li class="nex_favbtn"><a href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum" id="a_favorite" class="nex_sc" title="{lang forum_favorite}" onclick="showWindow(this.id, this.href, 'get', 0);">收藏本版<i class="xi1" id="number_favorite" {if !$_G[forum][favtimes]} style="display:none;"{/if}><span class="number_favorite_num">$_G[forum][favtimes]</span></i></a></li>
                                    <div class="clear"></div>
                                </ul>
                            </div>
                            <div class="nex_nonebginterls_btmr"><a href="javascript:;" id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial'"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}">发表新帖</a></div>
                            <div class="clear"></div>
                        </div>
                        <div class="clear"></div>
                    </div>
                ...
                <div class="nex_nonebgpart_btm">
                    <!--{if $_G['page'] == 1 && $_G['forum']['rules']}-->
                    <div id="forum_rules_{$_G[fid]}" style="$collapse['forum_rules'];">
                        <div class="nex_noneguize"><em>板块规则</em>$_G['forum'][rules]</div>
                    </div>
                    <!--{else}-->
		...
		<div class="sd">
            ...
            {if file_exists('source/plugin/nex_decos_180401/inc.php')}
            <!--{eval require_once("source/plugin/nex_decos_180401/inc.php");}-->
            <div class="nex_vt_formpost">
                <!--申请表单提交开始-->
                {$form[0]}
                <!--申请表单提交结束-->
            </div>
