					...
					<div class="nex_biaotou">
                        ...
                        <div class="y nex_space">
                        	<span id="clearstickthread" style="display: none;">
                                <a href="javascript:;" onclick="clearStickThread()" class="nex_showdisplays" title="{lang showdisplayorder}">{lang showdisplayorder}</a>
                            </span>
                            ...
                            <!--{else}-->
                            <a{if empty($_G['cookie']['forumdefstyle'])} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=yes" class="nex_wfpower chked"{else} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=no" class="nex_wfpower unchk"{/if} title="{lang view_thread_imagemode}{lang view_thread}">{lang view_thread_imagemode}</a>
                            <!--{/if}-->
                        </div>
					</div>
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
                <!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
						<div class="nex_ancmtlist">
							<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><!--{/if}-->
							<div class="nex_announcement">{lang announcement}: </div>
							...
							<div class="nex_anmtauthor"><a href="home.php?mod=space&uid=$announcement[authorid]" c="1">$announcement[author]</a></div>
							<div class="nex_anmtdate">	<em>$announcement[starttime]</em></div>
                            <div class="clear"></div>
						</div>
				<!--{/if}-->
							...
							<!--{if $_G[setting][forumseparator] == 1 && $separatepos == $key + 1}-->
								<tbody id="separatorline">
									<tr class="ts" style=" height: 30px; line-height: 30px;">
										...
										<th style="border-right: 1px solid #F3F3F3;"><!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang forum_thread}</a><!--{else}-->&nbsp;<!--{/if}--></th>
									</tr>
								</tbody>
								...
								<tr>
									...
									<th class="$thread[folder] forumtit">
										...
										<!--{elseif $thread['special'] == '3' && $thread['price'] < 0}-->
											- <a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=specialtype&specialtype=reward$forumdisplayadd[specialtype]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}&rewardtype=2" title="{lang show_rewarded_only}" style="margin-right:5px;">[{lang reward_solved}]</a>
										<!--{/if}-->
										<!--{if $thread['attachment'] == 2}-->
											<img src="$_G['style'][styleimgdir]/forumlist/image_s.gif" alt="attach_img" title="{lang attach_img}" align="absmiddle" />
										<!--{elseif $thread['attachment'] == 1}-->
											<img src="$_G['style'][styleimgdir]/forumlist/common.gif" alt="attachment" title="{lang attachment}" align="absmiddle" />
										<!--{/if}-->
										<!--{if $thread['mobile']}-->
											<img src="{IMGDIR}/mobile-attach-$thread['mobile'].png" alt="{lang post_mobile}" align="absmiddle" />
										<!--{/if}-->
										<!--{if $thread['digest'] > 0 && $filter != 'digest'}-->
											<img src="$_G['style'][styleimgdir]/forumlist/digest_$thread[digest].png" align="absmiddle" alt="digest" title="{lang thread_digest} $thread[digest]" />
										<!--{/if}-->
										<!--{if $thread['displayorder'] == 0}-->
											...
											<!--{/if}-->
											<!--{if $thread[heatlevel]}-->
												<img src="$_G['style'][styleimgdir]/forumlist/hot_$thread[heatlevel].gif" align="absmiddle" alt="heatlevel" title="{lang heats}: {$thread[heats]}" />
											<!--{/if}-->
											<!--{if $thread['rate'] > 0}-->
												<img src="$_G['style'][styleimgdir]/forumlist/agree.gif" align="absmiddle" alt="agree" title="{lang rate_credit_add}" />
											<!--{elseif $thread['rate'] < 0}-->
												<img src="$_G['style'][styleimgdir]/forumlist/disagree.gif" align="absmiddle" alt="disagree" title="{lang posts_deducted}" />
											<!--{/if}-->
										<!--{/if}-->
										...
										<!--{elseif in_array($thread['displayorder'], array(1, 2, 3, 4))}-->
											<i class="nex_forumlist_icon_adj"><img src="$_G['style'][styleimgdir]/forumlist/pin_$thread[displayorder].png" alt="$_G[setting][threadsticky][3-$thread[displayorder]]" /></i>
										<!--{else}-->
											<img src="$_G['style'][styleimgdir]/forumlist/folder_$thread[folder].png" />
										<!--{/if}-->
										</a>
                                        ...
                                        <div class="nex_forum_authors">
                                        	<a class="nex_threads_author" href="home.php?mod=space&uid=$thread[authorid]" c="1"><i>by</i>$thread[author]</a>
                                        ...
                                        <div class="nex_threads_summary">
                                            ...
                                            <!--{echo cutstr($summary_echo,100)}-->
                                        </div>
                                        
                                        <div class="nex_forumlist_pics">
                                        	<!--{eval include 'template/nex_deco_180401/php/nex_innerpics.php'}-->
											<!--{loop $nex_localpic $nex_pic}-->
                                        	<a href="forum.php?mod=viewthread&tid=$thread[tid]" target="_blank">
                                            	<img src="data/attachment/forum/$nex_pic[attachment]">
                                            </a>
                                            <!--{/loop}-->
                                            <div class="clear"></div>
                                        </div>
                                        ...
                                        <div class="foruminfo"> 
                                        <i class="z">
                                        	<span class="nex_view"><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></span>
                                            <span class="nex_reply">$thread[allreplies]</span>
										</i>
                                        
                                        <i class="y">
                                            
                                            <span class="nex_ftdate">$thread[dateline]</span>
                                        </i>
...
<!--{if $multipage && $filter != 'hot'}-->
    ...
    <div class="pgbtn">
    	<a class="bm_h" href="javascript:;" rel="$multipage_more" curpage="$page" id="autopbn" totalpage="$maxpage" picstyle="$_G[forum][picstyle]" forumdefstyle="$_G[cookie][forumdefstyle]">
        	<em></em>
	...
	<!--{else}-->
		<div id="pgbtn" class="pgbtn">
       		<div class="nex_wf_ajax_pn">
        		<a href="forum.php?mod=forumdisplay&fid={$_G[fid]}&filter={$filter}&orderby={$_GET[orderby]}{$forumdisplayadd[page]}&{$multipage_archive}&page=$nextpage" hidefocus="true">下一页</a>
            </div>
        </div>
	<!--{/if}-->
<!--{/if}-->
