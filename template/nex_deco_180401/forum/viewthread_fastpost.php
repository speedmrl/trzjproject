	...
	<table cellspacing="0" cellpadding="0">
		<tr>
			<td class="pls">
                ...
                <!--{else}-->
                <div class="nex_unlog_avator"><img src="$_G['style'][styleimgdir]/viewthread/default_avator1.png" /></div>
                <!--{/if}-->
		...
		<div{if empty($_GET[from]) && $_G[setting][fastsmilies]} class="hasfsl"{/if} id="fastposteditor">
			<div class="tedt{if !($_G[forum_thread][special] == 5 && empty($firststand))} mtn{/if}">
				<div class="bar">
					...
					<!--{eval $seditor = array('fastpost', array('at', 'bold', 'color', 'img', 'link', 'quote', 'code', 'smilies'), !$allowfastpost ? 1 : 0, $allowpostattach && $_GET['from'] != 'preview' && $allowfastpost ? '<span id="spanButtonPlaceholder">'.lang('template', 'upload').'</span>' : '');}-->
