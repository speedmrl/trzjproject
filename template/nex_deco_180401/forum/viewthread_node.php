	...
	<td class="pls" rowspan="2">
		<div id="favatar$post[pid]" class="pls favatar">
			...
			<!--{elseif getstatus($post['status'], 5)}-->
				...
				<!--{/if}-->
			<!--{else}-->
            	<div class="avatar"><img src="$_G['style'][styleimgdir]/viewthread/hidden.png" /></div>
				<div class="pi">
                    <div class="authi">
                    <!--{if !$post['authorid']}-->
		...
		<div class="pi">
			<!--{if !IS_ROBOT}-->
                ...
                <div class="foldcount y">
                    ...
                    <em>$_G[forum_thread][views]</em>
                    <span class="foldreplies"></span>
                    <em>$_G[forum_thread][allreplies]</em>
                </div>
                <!--{/if}-->
                <!--{if !$close_leftinfo || !$post['first']}-->
				<strong>
			...
			<div class="pti">
                ...
                <!--{if $post['first'] &&  $_G['forum_threadstamp']}-->
                    <span class="threadstamp">$_G[forum_threadstamp][text]</span>
                <!--{/if}-->
				...
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					...
					<!--{/if}-->
					<!--{if $close_leftinfo}--><a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2">$post[author]</a>$authorverifys<!--{/if}-->
					...
					<!--{/if}-->
				<!--{elseif getstatus($post['status'], 5)}-->
					<!--{if $close_leftinfo}--><a href="javascript:;" class="xi2">$post[author]</a><!--{/if}-->
					&nbsp;<em id="authorposton$post[pid]">{lang poston} $post[dateline]</em>
				...
				<!--{/if}-->
				<!--{if $post['first']}-->
				<span class="pipe {if !$close_leftinfo}show{/if}">|</span><a href="javascript:;" onclick="readmode($('thread_subject').innerHTML, $post[pid]);" class="{if !$close_leftinfo}show{/if}">{lang read_mode}</a>
				<!--{/if}-->
				<!--{if $close_leftinfo && $post['first']}-->
				<span class="pl10">
	...
	<tr><td class="plc plm">
			...
			<!--{if $post['invisible'] == 0}-->
				<div id="p_btn" class="mtw mbm hm cl">
					...
					<!--{if $_G['group']['raterange'] && $post['authorid']}-->
                        ...
                        <img src="$_G['style'][styleimgdir]/viewthread/agree.png" alt="{lang rate}" />
                        <div class="nex_icon_bottom"><span class="nex_sc">{lang rate}</span></div>
                        </a>
					<!--{/if}-->
					<!--{if !$post['anonymous'] && $post['first'] && helper_access::check_module('follow')}-->
						<a class="followp" href="home.php?mod=spacecp&ac=follow&op=relay&tid=$_G[tid]&from=forum" onclick="showWindow('relaythread', this.href, 'get', 0);" title="{lang follow_spread}"><img src="$_G['style'][styleimgdir]/viewthread/zhuanbo.png" alt="{lang thread_realy}" />
                        <div class="nex_icon_bottom"><span class="nex_sc">{lang thread_realy}</span><!--{if $_G['forum_thread']['relay']}--><span id="relaynumber" >{$_G['forum_thread']['relay']}</span><!--{/if}--></div>
                        
                        </a>
					<!--{/if}-->
					<!--{if $post['first'] && helper_access::check_module('share')}-->
						<a class="sharep" href="home.php?mod=spacecp&ac=share&type=thread&id=$_G[tid]" onclick="showWindow('sharethread', this.href, 'get', 0);" title="{lang share_digest}"><img src="$_G['style'][styleimgdir]/viewthread/share.png" alt="{lang thread_share}" /><div class="nex_icon_bottom"><span class="nex_sc">{lang thread_share}<!--{if $_G['forum_thread']['sharetimes']}--></span><span id="sharenumber">{$_G['forum_thread']['sharetimes']}</span><!--{/if}--></div></a>
					<!--{/if}-->

					<!--{if !$_G['forum']['disablecollect'] && helper_access::check_module('collection')}-->
						<a href="forum.php?mod=collection&action=edit&op=addthread&tid=$_G[tid]" id="k_collect" onclick="showWindow(this.id, this.href);return false;" onmouseover="this.title = $('collectionnumber').innerHTML + ' {lang activity_member_unit}{lang collection}'" title="{lang thread_collect}"><img src="$_G['style'][styleimgdir]/viewthread/taotie.png" alt="{lang thread_share}" />
                        
                        <div class="nex_icon_bottom"><span class="nex_sc">{lang collection}</span><span id="collectionnumber"{if !$post['releatcollectionnum']} {/if}>{$post['releatcollectionnum']}</span></div>
                        
                        </a>
					<!--{/if}-->
					<!--{if ($_G['group']['allowrecommend'] || !$_G['uid']) && $_G['setting']['recommendthread']['status']}-->
						<!--{if !empty($_G['setting']['recommendthread']['addtext'])}-->
						<a id="recommend_add" href="forum.php?mod=misc&action=recommend&do=add&tid=$_G[tid]&hash={FORMHASH}" {if $_G['uid']}onclick="ajaxmenu(this, 3000, 1, 0, '43', 'recommendupdate({$_G['group']['allowrecommend']})');return false;"{else} onclick="showWindow('login', this.href)"{/if} onmouseover="this.title = $('recommendv_add').innerHTML + ' {lang activity_member_unit}$_G[setting][recommendthread][addtext]'" title="{lang maketoponce}"><img src="$_G['style'][styleimgdir]/viewthread/check.png" alt="$_G['setting']['recommendthread'][addtext]" /><div class="nex_icon_bottom"><span class="nex_sc">$_G['setting']['recommendthread'][addtext]</span><span id="recommendv_add"{if !$_G['forum_thread']['recommend_add']} {/if}>$_G[forum_thread][recommend_add]</span></div></a>
						<!--{/if}-->
						<!--{if !empty($_G['setting']['recommendthread']['subtracttext'])}-->
						<a id="recommend_subtract" href="forum.php?mod=misc&action=recommend&do=subtract&tid=$_G[tid]&hash={FORMHASH}" {if $_G['uid']}onclick="ajaxmenu(this, 3000, 1, 0, '43', 'recommendupdate(-{$_G['group']['allowrecommend']})');return false;"{else} onclick="showWindow('login', this.href)"{/if} onmouseover="this.title = $('recommendv_subtract').innerHTML + ' {lang activity_member_unit}$_G[setting][recommendthread][subtracttext]'" title="{lang makebottomonce}"><img src="$_G['style'][styleimgdir]/viewthread/close.png" alt="$_G['setting']['recommendthread'][subtracttext]" /><div class="nex_icon_bottom"><span class="nex_sc">$_G['setting']['recommendthread'][subtracttext]</span><span id="recommendv_subtract"{if !$_G['forum_thread']['recommend_sub']} {/if}>$_G[forum_thread][recommend_sub]</span></div></a>
						<!--{/if}-->
					<!--{/if}-->
		...
		<!--{if $post['relateitem']}-->
			<div class="nex_mean mtw mbw">
				<h3 class="pbm mbm bbda nex_relatedthreads"><span>{lang related_thread}</span></h3>
				<ul class="xl xl2 cl nex_threadslists">
					<!--{loop $post['relateitem'] $var}-->
					<li>
                    	<a href="forum.php?mod=viewthread&tid=$var[tid]" title="$var[subject]" target="_blank">	
                            $var[subject]
                        </a>
                    </li>
					<!--{/loop}-->
...
<!--{/if}-->
<!--{if $post['first'] && $close_leftinfo && $_G[forum_thread][allreplies]}-->
<div class="allrepliestit" id="detail-tag3">
	<div class="allrepliesw cl">
    	<h2 class="z">精彩评论<em>$_G[forum_thread][allreplies]</em></h2>
        <!--{if !IS_ROBOT && !$_G['forum_thread']['archiveid'] && $post['first'] }-->
        <!--{if !$postcount }-->
            <div id="fj" class="y">
                ...
                <a href="javascript:;" id="fj_btn" class="z" title="{lang thread_redirect_postno_tips}"><img src="$_G['style'][styleimgdir]/viewthread/fj_btn.png" alt="{lang thread_redirect_postno_tips}" class="vm" /></a>
            </div>
        <!--{/if}-->
        <div class="reply_order y">
        	<!--{if !$rushreply}-->
				<!--{if $ordertype == 1}-->
                    <a onmouseover="showMenu(this.id)" class="showmenu" href="javascript:;" id="reply_order">{lang post_descview}</a>
                <!--{else}-->
                    <a onmouseover="showMenu(this.id)" class="showmenu" href="javascript:;" id="reply_order">{lang post_ascview}</a>
                <!--{/if}-->
            <!--{/if}-->
        </div>
         <!--{/if}-->
...
<div id="reply_order_menu" style=" display:none;">
    ...
    <!--{if !IS_ROBOT && !$_G['forum_thread']['archiveid'] && $post['first'] }-->
        <!--{if !$rushreply}-->
                <li><a href="forum.php?mod=viewthread&tid=$_G[tid]&extra=$_GET[extra]&ordertype=1"  class="show">{lang post_descview}</a></li>
                <li><a href="forum.php?mod=viewthread&tid=$_G[tid]&extra=$_GET[extra]&ordertype=2"  class="show">{lang post_ascview}</a></li>
        <!--{/if}-->
    <!--{/if}-->
    </ul>
</div>
