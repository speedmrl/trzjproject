...
<!--{else}-->
	<table cellspacing="0" cellpadding="0" class="nex_post_tb tfm">
	<!--{if $_G['forum']['threadsorts']['description'][$_G['forum_selectsortid']]}-->
		<tr>
			<th class="ptm pbm bbda nex_sort_Names">{lang threadtype_description}</th>
			<td class="ptm pbm bbda nex_sort_Descs" colspan="2">$_G[forum][threadsorts][description][$_G[forum_selectsortid]]</td>
		</tr>
	<!--{/if}-->
	<!--{if $_G['forum']['threadsorts']['expiration'][$_G['forum_selectsortid']]}-->
		<tr>
	...
	<!--{loop $_G['forum_optionlist'] $optionid $option}-->
		<tr>
			<th class="ptm pbm bbda nex_post_th"><!--{if $option['required']}--><span class="rq">*</span><!--{/if}-->$option[title]</th>
			<td class="ptm pbm bbda nex_post_td">
				<div id="select_$option[identifier]" class="nex_selections">
				<!--{if in_array($option['type'], array('number', 'text', 'email', 'calendar', 'image', 'url', 'range', 'upload', 'range'))}-->
					...
					<!--{elseif $option['type'] == 'image'}-->
						...
						<!--{/if}-->
						<div id="sortattach_image_{$option[identifier]}" class="ptn nex_uploading_IMG">
						<!--{if $option['value']['url']}-->
